...
4. 各行の「配置」を押すと、現在の再生ヘッド位置へ上書き配置されます。

JSON読込時には、各`file_path`とPremiere Proプロジェクト内のProjectItemを完全パスで照合します。プロジェクト内に存在しない素材はグレーアウトし、配置できません。素材をPremiere Proへ読み込んだ後、「再照合」を押すと配置可能になります。
